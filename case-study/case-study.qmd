...
The aim of the case study is to analyse the explanation days of absenteeism (`daysabs`) of 316 students from two Los Angeles high schools (`school`) using their characteristics such as gender (`gender`), ethnicity (`ethnic`), and standardised math (`mathnce`) and language scores (`langnce`). Example rows from the dataset are presented below


```{r}
df <- read_excel("../data/school-absence-for-lecture.xlsx")
head(df)
```
...
3. plot `daysabs` by `gender`, `ethnicity`, `school`, `maths` and `language` to assess the relationship,
