...
2. `summary(goodfit object)` returns a $\chi^2$ and $G^2$ tests using `cdf` of a given distribution. 3. Pearson residuals are calcuated as 
...
```{r}
X_dt <- data.frame(X = 0:4, n_hat = n_hat, r_hat = r_hat)
X_dt
```
Check if negative binomail distribution fits the data better.
...
```
Now, let's see what LR test will tell. For this example it seems that two distrubutions equally well fit the data.
...
X_dt = pd.DataFrame({'X': np.arange(0, 5), 'n_hat': n_hat, 'r_hat': r_hat})
X_dt
```
```{python}
## loglik for n
